...
typedef enum
{
	...
	EWS_PIXEL_FORMAT_NV21      = 14,
	EWS_PIXEL_FORMAT_I420      = 15,
} EWS_PIXELFORMAT;

#define EWS_PIXEL_FORMAT_MAX EWS_PIXEL_FORMAT_I420
...
typedef struct EWS_EVENT_TAG
{
	...
	union
	{
		/* each union member must be 32 bits */
		unsigned long uiEventData; /* nothing */
		EWS_ATOM sPropertyName;    /* (R)PROPERTY_NOTIFY */
		unsigned long uiKeyCode;   /* KEYPRESS */
		EWS_ATOM sMessage;         /* CLIENT_MESSAGE */
	};
} EWS_EVENT;
...
/* The keycodes from an associated EWS_EVENT_KEYPRESS event
   These are the minimum to get the SDK demo working.
